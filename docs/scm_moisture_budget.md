...
Cloud-handoff experiments then removed the direct large-scale precipitation fraction so saturation adjustment transferred condensed vapor to the prognostic cloud reservoir before rain formation. This eliminated an arbitrary precipitation label and made saturated mass and diagnosed boundary-layer depth more consistent in short screens, but exposed strong resolution dependence in the local quadratic autoconversion sink. A first-order local sink reduced that sensitivity only partly. The experimental `mf_edmf_column_autoconv_v24.toml` instead converts excess column cloud-water path on a fixed timescale and removes condensate proportionally from cloudy layers; a component test verifies identical removal and exact water conservation for the same cloud path at 20, 40, and 80 levels.

In the five-day v24 screen, cloud precipitation was 1.14, 2.03, and 2.15 mm day-1 and total precipitation was 1.84, 2.48, and 2.40 mm day-1 at 20, 40, and 80 levels. The close 40- and 80-level agreement shows that column-path autoconversion removes most of the fine-grid sensitivity, while the dry 20-level result shows that condensate generation or plume detrainment remains under-resolved on the production grid. No multi-resolution slab equilibria are warranted. Resolution development should continue with short fixed-SST or component tests; once a candidate passes, only the production 20-level configuration needs the expensive slab-ocean equilibrium integration.

Process-source diagnostics localized the remaining coarse-grid error before detrainment. In a three-day v24 screen, the 20-level plume reached only 56 m, never became cloudy, and produced no plume-condensate source. The 40-level plume reached 413 m with a 313 m cloud base, while the 80-level plume reached 892 m with a 325 m cloud base. Large-scale cloud sources were 0.11, 0.31, and 0.75 mm day-1. Explicitly depositing condensate during plume substeps did not create 20-level condensate and greatly exaggerated fine-grid recycling, so it was rejected. A mass-weighted 300 m launch layer suppressed the buoyant plume at every resolution, while a convective velocity based directly on surface buoyancy flux caused an unacceptable fine-grid energy residual; both were also rejected and reverted. The evidence points to resolution-sensitive source-environment reconstruction and plume buoyancy below cloud base. A replacement should use a benchmarked subgrid updraft distribution or an internal plume grid and must pass BOMEX, conservation, and 20-level response tests before a single 20-level slab equilibrium is run.

The experimental `mf_edmf_internal_launch_v26.toml` treats the plume as a surface-driven thermal through a 350 m internal launch layer before allowing sampled negative buoyancy to terminate it. This left resolved BOMEX unchanged and corrected the ATM407 20-level cutoff: in the five-day screen the 20-level plume reached 1514 m and formed cloud, while total precipitation was 2.10, 2.55, and 2.53 mm day-1 at 20, 40, and 80 levels. A single 400-day 20-level slab-ocean adjustment was therefore run from the accepted v4 checkpoint. It produced surface temperature 284.90 K, TOA imbalance +3.00 W m-2, surface imbalance +2.70 W m-2, final-window drift 0.054 K, total precipitation 3.56 mm day-1, cloud precipitation 2.52 mm day-1, CAPE 1096 J kg-1, and RH95 mass fraction 26%. Water, energy, and MSE closure remained excellent and no limiter activated, but the cloud-water handoff overcorrected the production column and the energy balance failed. V26 is retained only as a diagnostic candidate; it must not replace the default or notebook checkpoint.

A 20-level handoff calibration from the v26 checkpoint showed that sending 25%, 50%, or 75% of saturation-adjusted condensate directly to precipitation only relabeled the rain and slightly increased total precipitation to 3.61-3.68 mm day-1; it did not improve the energy balance. Increasing the column-path autoconversion threshold from 0.02 to 0.20 kg m-2 reduced total precipitation to 3.29 mm day-1 and TOA imbalance to +2.65 W m-2, but increased retained cloud water from 0.15 to 0.33 kg m-2. A threshold of 0.80 kg m-2 brought the ten-day TOA diagnostic to +0.53 W m-2, but only by accumulating 0.92 kg m-2 of cloud water while leaving a +3.47 W m-2 surface imbalance. These settings are physically indefensible and were rejected without a longer equilibrium run. The excess cloud rain therefore cannot be repaired by calibrating the precipitation split or autoconversion constants; the next target is the upstream production and recycling of grid-scale condensate by plume detrainment, saturation adjustment, and boundary-layer transport.

Process-partition diagnostics from the v26 checkpoint showed that the plume deposits 2.68 g kg-1 day-1 of condensate into the single 750 hPa model layer while local TKE transport removes 0.90 g kg-1 day-1 there. Over the column, plume condensate production is 1.64 mm day-1 and subsequent saturation adjustment adds 1.30 mm day-1; evaporation returns only 0.42 mm day-1 before 2.52 mm day-1 autoconverts to rain. The EDMF conservation correction is negligible, and a dedicated test confirmed that the plume surface-flux fraction controls its thermodynamic anomaly rather than adding a second surface source. Reducing that fraction from 25% to 10-15% lowered saturated mass from 26% to 20%, but total precipitation remained 3.44-3.49 mm day-1 and the surface imbalance worsened to +3.56 to +4.11 W m-2. The v26 development line is therefore closed rather than subjected to further tuning. The accepted `mf_response_v3` default and checkpoint remain the coupled-model baseline; `mf_profile_v4` is a separate ozone-profile experiment and should not be described as the production default.

Comparison with the public CLUBB implementation, the BOMEX LES intercomparison, and the GFDL AM4 physics description showed that the experimental EDMF line omitted structures on which those established schemes depend. CLUBB predicts total water, liquid-water potential temperature, and subgrid moments on a boundary-layer grid, while AM4 uses a separately closed shallow plume with stronger lateral mixing. The reference BOMEX calculation uses about 40 m vertical spacing through a 3.5 km domain; the global 20-level sigma grid has only seven levels below 2 km. A two-component plume/environment cloud-water diagnosis produced a realistic 0.017 kg m-2 cloud-water path and 8.8% peak layer cloud fraction at 80 levels, but failed at 20 and 40 levels and did not improve the complete 20-level ATM407 column. It was rejected and removed. Future EDMF development must use a dedicated boundary-layer subgrid or prognostic PDF moments and pass quantitative BOMEX profile gates before coupling to ATM407.

The actual production suite was rechecked independently by restarting `notebooks/data/atm407_equilibrium_20level.npz` under `scm/configs/default.toml` for ten days. It produced 0.0016 K surface-temperature drift, +0.68 W m-2 TOA balance, +0.39 W m-2 surface balance, 1.92 mm day-1 total precipitation, 1.68 mm day-1 deep precipitation, 1441 J kg-1 CAPE, 3.5% saturated mass, and water, energy, and MSE residuals within the production tolerances without activating a limiter. `scripts/diagnose_atm407_budget.py --production-gates` now checks these requirements directly. This validated simple suite is appropriate for current dycore integration even though the higher-order shallow-cloud research scheme is not ready.
